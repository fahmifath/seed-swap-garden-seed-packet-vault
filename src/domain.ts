...
export interface ValidationErrors{[key:string]:string|undefined;plantName?:string;source?:string;packetYear?:string;quantity?:string;}
export interface ValidateResult{ok:boolean;errors:ValidationErrors;}
export interface RawInput{plantName:string;source:string;packetYear:string;quantity:string;notes:string;}
...
return{id,plantName:input.plantName.trim(),source:input.source as Source,packetYear:Number(input.packetYear),quantity:input.quantity as Quantity,notes:input.notes.trim().slice(0,NOTES_MAX_LEN),createdAt:now};
}
...
if(!id)return null;
return{id,plantName:input.plantName.trim(),source:input.source as Source,packetYear:Number(input.packetYear),quantity:input.quantity as Quantity,notes:input.notes.trim().slice(0,NOTES_MAX_LEN),createdAt:typeof r.createdAt==="string"?r.createdAt:new Date(0).toISOString()};
}
...
export function sortByExpiry(items:SeedItem[]):SeedItem[]{return[...items].sort((a,b)=>a.packetYear-b.packetYear);}
...
const s=getExpiryStatus(packetYear,currentYear);
if(s==="expired")return`${packetYear} (expired)`;
if(s==="expiring-soon")return`${packetYear} (expires this year)`;
