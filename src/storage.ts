...
export type LoadResult={status:"ok";items:SeedItem[]}|{status:"empty"}|{status:"partial";items:SeedItem[];message:string}|{status:"error";message:string};
...
let skipped=0;
for(const entry of parsed){const item=normalize(entry);if(item)valid.push(item);else skipped++;}
...
try{localStorage.setItem(KEY,JSON.stringify(items));return{status:"ok"};}
catch(err){
if(err instanceof DOMException&&err.name==="QuotaExceededError")return{status:"error",message:"Storage quota exceeded. Remove some packets to free up space."};
